...
namespace LeaveManagement.Application.Repositories
{
    public class LeaveRequestRepository : GenericRepository<LeaveRequest>, ILeaveRequestRepository
    {
        ...
        public async Task<bool> CreateLeaveRequest(LeaveRequestCreateVM model)
        {
            ...
            var start = (int)(model.StartDate.Value).Day;
            var end = (int)(model.EndDate.Value).Day;
            var requests = await GetAllAsync();
            foreach (var request in requests)
            {
                var startRequest = (int)(request.StartDate).Day;
                var endRequest = (int)(request.EndDate).Day;
                if ((startRequest) == start && (end == endRequest))
                {
                    return false;
                }
            }
            ...
            if (daysRequested <= 0)
            {
                leaveRequest.SelectedHours = model.SelectedHours.Value;
            } else
            {
                leaveRequest.SelectedHours = 0;
            }
